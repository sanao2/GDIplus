...
void Texture::Update() //나중에 애니메이션 상태에 따라서 계속해서 이미지를 바꿔줄 옵션 (반복문) 
{
	...
	GDIplusLoadImage(curRectivePath);
}
...
//GDI plus Load
void Texture::GDIplusLoadImage(const char* AnimPath) // 가상 함수를 통해서 애니메이션 프레임의 객체 마다 다른 이미지를 부를 수 있게 해볼 수 있을까? 
{
	
	ImageBitmap = new Gdiplus::Bitmap(AnimPath);
