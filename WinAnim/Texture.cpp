...
void Texture::Update() //나중에 애니메이션 상태에 따라서 계속해서 이미지를 바꿔줄 옵션 (반복문) 
{
	float FrameDeltaTime = Time::GetDeltaTime();  // 프레임 실시간 재생 시간 
...
void Texture::GDIplusRender()
{
	...
	BackBufferGraphics->DrawImage(ImageBitmap, Gdiplus::Rect(0, 0, ImageBitmap->GetWidth(), ImageBitmap->GetHeight()));
	GetImgWidth();
	GetImgHeight();
...
//GDI plus Load
void Texture::GDIplusLoadImage() // 가상 함수를 통해서 애니메이션 프레임의 객체 마다 다른 이미지를 부를 수 있게 해볼 수 있을까? 
{
	WCHAR path[MAX_PATH];
	GetCurrentDirectoryW(MAX_PATH, path);
	wcscat_s(path, L"\\Resources\\elf32.png");

	MessageBoxW(hWnd_, path, L"로드 경로 확인", MB_OK);
